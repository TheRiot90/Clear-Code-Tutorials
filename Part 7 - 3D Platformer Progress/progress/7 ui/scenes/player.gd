...
func set_hearts(value: int):
	for heart in $UI/HFlowContainer.get_children():
		heart.queue_free()
	for i in value:
		var heart = heart_scene.instantiate()
		$UI/HFlowContainer.add_child(heart)
...
func _on_jump_detection_area_body_entered(enemy: Node3D) -> void:
	if velocity.y < 0:
		enemy.defeat()
